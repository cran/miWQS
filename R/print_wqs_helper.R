...
#'@keywords wqs
#'
#'@description Prints the fitted WQS model along with the mean weights. Adjusted from print.lm.
#'
#'@param x  An object of class WQS, from \code{\link{estimate.wqs}}.
#'@inheritParams base::print
#'
#'@examples
#' #See \code{\link{estimate.wqs}}.
...
print.wqs<-function(x, digits = max(3L, getOption("digits") - 3L), ...){
    ...
    ##Summarize by family
      ...
      if(W$call$family == "gaussian"){
        ...
        se <- WQS.regression[ ,2]
        Ninety.Five.CI <- suppressWarnings( formatCI( cbind(
          mean = coef(W),
    ...
    ##Tell about convergence.
      ...
      if ( B ==  sum(d$convergence) ) {
        print( paste0("All (", B, ") bootstraps have converged."))
      } else{
        print(paste0(B - sum(d$convergence), " bootstrap(s) failed to converged. Those are: "))
