...
#' @description Combine multiple parameter estimates (as used in MI) across the K imputed complete
...
#'
#' @note  Modified the \code{\link[mice]{pool.scalar}} (version R 3.4) in the \pkg{mice} package to
...
#' @param n        A number providing the sample size. If nothing is specified, a large sample n = 99999 is assumed. Used in calculating the degrees of freedom; has no effect if K = 1.
...
#' @param alpha    Type I error used in forming confidence interval. Default: 0.05.
#' @inheritParams estimate.wqs
      #verbose
#' @return A dataframe consisting:
#' \describe{
...
pool.mi<- function(to.pool, n = NULL, method = c("smallsample","rubin"), alpha = 0.05, verbose = FALSE){
  method <- match.arg(method)

    ##Information
    p<-dim(to.pool)[[1]]  #Number of parameters
    if( dim(to.pool)[[2]] != 2){ stop( "There are more than two statistics. Please reformat to put mean and standard error in order.")}
    if (is.null(n)) {
      n<- 999999
    ...
    if( K == 1 ){
      cat( "There is one imputation; so the orginal estimates are used. The degrees of freedom are assumed to be 1E-9 so test used is normal (see glm2).")
    ...
    if(verbose) {   # Formula 3.1.9 from Rubin 1987
...
# #'@title Calculating Degrees of Freedom
...
mice.df<-function (m, lambda, dfcom, method){
  ...
  dfold <- (m - 1)/lambda^2  # Or, in terms of r (rel increase in variance):
                             #  (m - 1) * (1 + (1/r) )^2
  if (method != "smallsample"){
    df <- dfold
  } else{
    dfobs <- (dfcom + 1)/(dfcom + 3) * dfcom * (1 - lambda)  #or  (dfcom + 1)/(dfcom + 3) * dfcom * 1/(r+1) )
...
####Save functions
#save.image("./stage_3_pool_mi_function.RData")
